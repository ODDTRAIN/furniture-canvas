...
const BlockAssembler = ({ width, rows, columns, unitId, blockId, accessories, activeTool, onCellClick, isNightMode }) => {
  let currentY = 0;
  const elements = [];
  const totalH = rows.reduce((a, b) => a + b, 0) + (rows.length + 1) * WOOD_THICK;
    ...
    elements.push(
      <group key={`row-${rIdx}`} position={[0, currentY + h/2, 0]}>
        ...
        {Array.from({ length: columns }).map((_, c) => {
          ...
          if (isNightMode) elements.push(<CabinetLight key={`light-${rIdx}-${c}`} width={INNER_WIDTH} position={[centerX, h/2 - 0.01, 0]} />);
          return <CellSpace key={`cell-${c}`} width={INNER_WIDTH} height={h} position={[centerX, 0, 0]} accessoryData={accessories?.[cellKey]} activeTool={activeTool} isValid={isRowValid} onInteract={() => onCellClick(unitId, cellKey, h)} />;
        })}
      </group>
    );
    currentY += h;
...
// 가구 태그 스타일
const tagStyle = { pointerEvents: "none", background: "#1d1d1f", color: "#fff", padding: "4px 10px", borderRadius: "999px", fontSize: "10px", fontWeight: "600", boxShadow: "0 3px 6px rgba(0,0,0,0.25)", whiteSpace: "nowrap" };

export const UnitAssembler = ({ unit, position, ...props }) => {
  ...
  let curY = 0;
  return (
    <group position={position} onPointerOver={(e) => { if (!props.activeTool) { e.stopPropagation(); setHover(true); } }} onPointerOut={(e) => { if (!props.activeTool) { e.stopPropagation(); setHover(false); } }}>
      ...
      {unit.blocks.map(block => {
        ...
        const el = <group key={block.id} position={[0, curY, 0]}><BlockAssembler width={currentWidth} rows={block.rows} columns={unit.columns} unitId={unit.id} blockId={block.id} accessories={unit.accessories} {...props} /></group>;
        curY += h;
        return el;
      })}
      <Dimensions width={currentWidth} height={totalHeight} visible={props.showDimensions && (hovered || props.isSelected)} />
      {props.showNames && !(props.showDimensions && (hovered || props.isSelected)) && <Html position={[0, -0.25, 0]} center zIndexRange={[60, 0]}><div style={tagStyle}>{props.label}</div></Html>}
    </group>
  );
};